...
- **Создаёт задачи** в TickTick с нужным приоритетом и датой
...
>
> **Бот:** *создаёт задачу в TickTick*
>
> **Ты:** сделай высокий приоритет и добавь подзадачу на презентацию
>
> **Бот:** *обновляет задачу*

У агента доступ ко всему хранилищу, всем целям и TickTick. Спроси что угодно про свои записи - найдёт.
...
| «Какие приоритеты на эту неделю?» | Читает цели и TickTick, даёт прямой ответ |
...
2. **Исполнение** - создаёт задачи в TickTick, пишет файлы в хранилище, обновляет карточки
...
```
Telegram → Deepgram → Codex CLI → TickTick + Obsidian → Telegram отчёт
```
...
| Сервис | Цена |
|--------|------|
| OpenAI API (Codex) | По использованию |
| VPS (любой дешёвый) | ~$5/мес |
| Deepgram | Бесплатно ($200 кредит) |
| TickTick | Бесплатный план работает |
| **Итого** | **~$5/мес + расход OpenAI** |

Базовая инфраструктура остаётся дешёвой, а AI-стоимость зависит от реального использования.
...
- `vault/.codex/skills/dbrain-processor/references/about.md` - расскажи агенту о себе
- `vault/.codex/skills/dbrain-processor/references/classification.md` - как сортировать записи
...
| TickTick API Token | TickTick account settings | 1 мин |
...
| **ticktick-mcp** | Управление задачами, проектами, приоритетами |
...
| `.env` (`CODEX_CLI_COMMAND`) | Опционально: имя/путь до исполняемого файла Codex |
| `.memory-config.json` | Скорость угасания памяти, границы уровней |
| `mcp-config.json` | Подключения к внешним инструментам |
| `vault/.codex/AGENTS.md` | Личность и правила агента |
